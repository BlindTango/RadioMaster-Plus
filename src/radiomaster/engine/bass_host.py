...
"""
bass_host.py — RadioMaster+ BASS subprocess host.\n\nAdapted on 2026-09-05 from FreeRadio (https://github.com/Surveyor123/freeradio).\nCopyright remains with its contributors; RadioMaster+ modifications are\nCopyright (C) 2026 RadioMaster+ Team.
...
_BASS_ATTRIB_TEMPO_PITCH  = 0x10001   # BASS_FX pitch shift in semitones
...
# loaded into process. Now use the BASS_HLS_StreamCreateURL function
                ...
                if self._pitch_semitones:
                    try:
                        self._dll.BASS_ChannelSetAttribute(
                            ...
                            ctypes.c_float(self._pitch_semitones),
                        )
                    except Exception:
                        pass
            else:
    ...
    def media_seek(self, position_seconds):
        """Seek the current finite media stream to an absolute position."""
        with self._lock:
            handle, dll = self._handle, self._dll
        if not handle or not dll:
            return False, 0.0, 0.0
        try:
            ...
            if length_secs > 0:
                target = min(target, length_secs)
            target_bytes = dll.BASS_ChannelSeconds2Bytes(
                handle, ctypes.c_double(target))
            return (bool(dll.BASS_ChannelSetPosition(
                handle, target_bytes, _BASS_POS_BYTE)), target, length_secs)
        except Exception:
            return False, 0.0, 0.0

    def media_status(self):
        """Return current position and duration for any seekable stream."""
        return self.timeshift_status()
    ...
    def set_pan(self, pan):
        with self._lock:
            ...
            if self._handle and self._dll:
                try:
                    self._dll.BASS_ChannelSetAttribute(
                        self._handle, _BASS_ATTRIB_PAN,
                        ctypes.c_float(self._pan))
                except Exception:
                    pass

    def _apply_pan(self, handle):
        if handle and self._dll:
            try:
                self._dll.BASS_ChannelSetAttribute(
                    handle, _BASS_ATTRIB_PAN, ctypes.c_float(self._pan))
            except Exception:
                pass

    def set_device(self, device_index):
        """Select the BASS output device for subsequently opened streams."""
        if not self._dll:
            return False
        try:
            return bool(self._dll.BASS_SetDevice(int(device_index)))
        except Exception:
            return False
    ...
    def set_pitch(self, semitones):
        """Set pitch independently of tempo for tempo-wrapped media."""
        with self._lock:
            self._pitch_semitones = max(-12.0, min(12.0, float(semitones)))
            if not (self._tempo_active and self._handle and self._dll):
                return False
            try:
                return bool(self._dll.BASS_ChannelSetAttribute(
...
def main():
    ...
    dll_dir = os.environ.get("RADIOMASTER_BASS_ROOT") or os.path.dirname(os.path.abspath(__file__))
    ...
    for raw_line in sys.stdin:
        ...
        if cmd == "ping":
            ...
            host.set_pan(float(cmd_obj.get("value", 0.0)))
            _ok()

        elif cmd == "media_seek":
            seeked, position_seconds, length_seconds = host.media_seek(
                float(cmd_obj.get("position_seconds", 0.0)))
            _ok(seeked=seeked, position_seconds=position_seconds,
                length_seconds=length_seconds)

        elif cmd == "media_status":
            position_seconds, length_seconds = host.media_status()
            _ok(position_seconds=position_seconds,
                length_seconds=length_seconds)

        elif cmd == "set_device":
            changed = host.set_device(int(cmd_obj.get("index", 0)))
            _ok(changed=changed)
            ...
            applied = host.set_pitch(float(cmd_obj.get("semitones", 0.0)))
            _ok(applied=applied)
